...
| 2026-09-01 | Git履歴書き換えの見送り判断と、就職活動終了後のリポジトリ削除方針を記録 |
...
Git履歴に残る旧パスワード文字列については、`git-filter-repo`による書き換えを検討したが、以下の理由から実施を見送った。

- ブランチ保護ルールの一時解除が必要になる
- マージ済みPR(#17)のdiffはGitHub側に別途保持されており、履歴を書き換えても消えない可能性が高い
- 実害は既にローカルDBパスワードのローテーションで解消済みである

書き換えの手間・リスクに対して得られる効果が限定的と判断したため。就職活動終了後、本リポジトリ自体を削除する予定([今後の課題](#7-今後の課題)を参照)。
