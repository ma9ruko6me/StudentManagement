...
- バックエンド(Spring Boot): 既定ポート `8080`(`backend/src/main/resources/application.properties` に `server.port` の指定なし)
...
起動確認:
```bash
i=0; until curl -sf http://localhost:8080/students >/dev/null 2>&1 || [ $i -ge 60 ]; do sleep 1; i=$((i+1)); done
cat /tmp/backend-dev.log
```
(Spring Bootの起動には数十秒かかる場合がある。`/students`は認証不要の一覧検索APIのため疎通確認に使える)
